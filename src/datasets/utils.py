...
class CacheDataSet(torch.utils.data.Dataset):
    ...
    def preload_cache(self, n_workers=4, show_progress=True):
        """
        Preload the dataset into memory using multiprocessing.
        Populates self.audio_cache.
        """

        with multiprocessing.Pool(n_workers) as pool:
            args = [(idx, self.ds) for idx in range(len(self.ds))]
            imap_iter = pool.imap(_load_item_for_cache, args)
            if show_progress:
                imap_iter = tqdm(imap_iter, total=len(args), desc="Preloading cache")
            for idx, data in imap_iter:
                self.audio_cache[idx] = data
